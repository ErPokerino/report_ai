...
params:
  ...
  enable_ai: true
  ai_model: null  # null = usa default (gpt-4o), altrimenti specifica modello (es. "gpt-4o-mini", "gpt-4-turbo")
  ai_temperature: 0  # Temperatura per generazione LLM (0 = deterministico)
---
...
from ai_analysis import get_llm_with_fallback, get_model_display_name, get_ai_unavailable_message
from IPython.display import Markdown, display

# Controlli preventivi per evitare duplicati e sezioni vuote
_used_labels = set()  # Traccia label Quarto già utilizzati per grafici
_used_sections = set()  # Traccia titoli di sezioni già mostrati

def _check_label_unique(label: str) -> str:
    """
    Verifica se un label Quarto è unico, altrimenti genera una variante.
    
    Args:
        label: Label da verificare
        
    Returns:
        Label unico (originale o variante)
    """
    if label not in _used_labels:
        _used_labels.add(label)
        return label
    # Genera variante
    counter = 2
    while f"{label}-{counter}" in _used_labels:
        counter += 1
    unique_label = f"{label}-{counter}"
    _used_labels.add(unique_label)
    return unique_label

def _check_section_content(title: str, content: Any) -> bool:
    """
    Verifica che il contenuto di una sezione non sia None o vuoto prima di mostrarlo.
    Verifica anche che il titolo non sia già stato mostrato.
    ...
    Returns:
        True se la sezione può essere mostrata, False altrimenti
    """
    # Normalizza il titolo per il confronto (rimuovi markdown formatting)
    title_normalized = title.replace("###", "").replace("##", "").replace("#", "").strip()
    
    # Verifica duplicati
    if title_normalized in _used_sections:
        return False
    
    # Verifica contenuto vuoto
    if content is None:
        return False
    if isinstance(content, str) and (not content.strip() or content.strip() == ""):
        return False
    
    # Se passa i controlli, aggiungi ai titoli usati
    _used_sections.add(title_normalized)
    return True

# Determina quale modello LLM sarà utilizzato e ottieni il nome formattato
# Usa ai_model dal parametro se disponibile, altrimenti usa default
try:
    ai_model_param = params.ai_model if hasattr(params, 'ai_model') and params.ai_model else None
    ai_temp_param = params.ai_temperature if hasattr(params, 'ai_temperature') else 0
except (NameError, AttributeError):
    ai_model_param = None
    ai_temp_param = 0

llm_config = get_llm_with_fallback(primary_model=ai_model_param, temperature=ai_temp_param) if ai_model_param else get_llm_with_fallback(temperature=ai_temp_param)
if llm_config is not None:
    ...
    # Formatta la gerarchia di fallback
    if fallback_models:
        hierarchy = " → ".join([get_model_display_name(m) for m in fallback_models])
    else:
        hierarchy = model_display_name
else:
    model_display_name = "modelli LLM disponibili"
    hierarchy = "N/A (API keys non configurate)"
...
intro_text += "Le analisi e i commenti sono generati automaticamente utilizzando **LangChain** e modelli LLM. Il sistema utilizza la seguente gerarchia di modelli con fallback automatico: **" + hierarchy + "**. Se un modello non è disponibile o ha la quota esaurita, il sistema prova automaticamente il modello successivo nella gerarchia. Il modello effettivamente utilizzato per generare questo report viene indicato nella sezione 'Modello LLM Utilizzato' alla fine del documento."
...
try:
    ...
    if not enable_ai:
        ...
        # Usa llm_config globale se disponibile, altrimenti ottienilo
        if 'llm_config' not in globals():
            ai_model_param = params.ai_model if hasattr(params, 'ai_model') and params.ai_model else None
            ai_temp_param = params.ai_temperature if hasattr(params, 'ai_temperature') else 0
            llm_config = get_llm_with_fallback(primary_model=ai_model_param, temperature=ai_temp_param) if ai_model_param else get_llm_with_fallback(temperature=ai_temp_param)
        ...
        if llm_config is not None:
            ...
            ai_summary = get_ai_unavailable_message("no_config")
...
from ai_analysis import generate_chart_commentary, get_llm_with_fallback
from IPython.display import Markdown, display
...
try:
    enable_ai = params.enable_ai if hasattr(params, 'enable_ai') else True
    if enable_ai:
        # Ottieni llm_config se non già disponibile globalmente
        if 'llm_config' not in globals():
            ai_model_param = params.ai_model if hasattr(params, 'ai_model') and params.ai_model else None
            ai_temp_param = params.ai_temperature if hasattr(params, 'ai_temperature') else 0
            llm_config = get_llm_with_fallback(primary_model=ai_model_param, temperature=ai_temp_param) if ai_model_param else get_llm_with_fallback(temperature=ai_temp_param)
...
for field_name in field_names:
    ...
    if field_df['is_validated'].sum() > 0:
        ...
        if len(field_metrics) > 0:
            ...
            # Commento AI per le metriche (grafico aggregato disponibile nella sezione "Performance Metodi")
            try:
                enable_ai = params.enable_ai if hasattr(params, 'enable_ai') else True
                if enable_ai:
                    if 'llm_config' not in globals():
...
from ai_analysis import analyze_error_patterns, get_llm_with_fallback, get_ai_unavailable_message
from IPython.display import Markdown, display
...
try:
    ...
    if not enable_ai:
        ...
        if llm_config is not None:
            ...
            error_analysis = get_ai_unavailable_message("no_config")
except (NameError, AttributeError):
    if 'llm_config' not in globals():
        llm_config = get_llm_with_fallback()
    if llm_config is not None:
...
from ai_analysis import get_last_used_model, get_model_display_name, get_llm_with_fallback
from IPython.display import Markdown, display

# Ottieni informazioni sul modello utilizzato
used_model_info = get_last_used_model()
llm_config = get_llm_with_fallback()

if used_model_info:
    model_name = used_model_info.get("model_name")
    provider = used_model_info.get("provider", "unknown")
    model_display = get_model_display_name(model_name)
    provider_display = "OpenAI" if provider == "openai" else "Google" if provider == "google" else provider
    
    model_info_text = f"""
**Modello utilizzato:** {model_display}

**Provider:** {provider_display}

Il modello è stato selezionato automaticamente dalla gerarchia di fallback configurata in base alla disponibilità e alle quote API.
"""
else:
    # Se nessun modello è stato utilizzato, mostra la gerarchia configurata
    if llm_config is not None:
        fallback_models = llm_config.get("fallback_models", [])
        if fallback_models:
            hierarchy = " → ".join([get_model_display_name(m) for m in fallback_models])
...
*Nota: Nessuna chiamata LLM è stata effettuata durante la generazione di questo report. Le analisi AI potrebbero essere state disabilitate o non disponibili.*
"""
        else:
            model_info_text = "*Nessuna informazione disponibile sul modello LLM utilizzato.*"
    else:
        model_info_text = "*Analisi AI non disponibile: API keys non configurate.*"

display(Markdown(model_info_text))
```
...
from ai_analysis import generate_section_text, get_llm_with_fallback, get_ai_unavailable_message

recommendations = None
try:
    enable_ai = params.enable_ai if hasattr(params, 'enable_ai') else True
    if not enable_ai:
        recommendations = None  # Non mostrare sezione se AI disabilitata
    else:
        # Usa llm_config globale se disponibile, altrimenti ottienilo
        if 'llm_config' not in globals():
            ai_model_param = params.ai_model if hasattr(params, 'ai_model') and params.ai_model else None
...
"""
            ...
            if recommendations and (not recommendations.strip() or recommendations.strip() == ""):
                recommendations = None
        else:
            recommendations = None  # Non mostrare sezione se AI non disponibile
except (NameError, AttributeError):
    # params non disponibile, usa default
    if 'llm_config' not in globals():
        llm_config = get_llm_with_fallback()
...
"""
    ...
    recommendations = None  # Non mostrare sezione in caso di errore
```
...
# Mostra sezione solo se il contenuto non è vuoto
