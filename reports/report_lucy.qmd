...
intro_text += "Le analisi e i commenti sono generati automaticamente utilizzando **LangChain** e modelli LLM (" + model_display_name + ") per fornire insights strutturati sui dati."
...
from data_loader import load_csv_data, calculate_metrics_by_method, get_field_names, filter_by_field_name
...
if 'field_name' in df.columns:
    ...
    for fn in field_names:
        ...
        field_stats.append({
            ...
            '% Validati': f"{field_df['is_validated'].sum()/len(field_df)*100:.1f}%" if len(field_df) > 0 else "0%",
...
if method_usage_commentary:
    print(f"**Commento AI:** {method_usage_commentary}")
```
...
from data_loader import get_field_names, filter_by_field_name, calculate_metrics_by_method
...
for field_name in field_names:
    ...
    print(f"### Analisi: {field_name}")
    
    # Statistiche per questo campo
    field_summary = {
        'Totale record': f"{len(field_df):,}",
        'Record validati': f"{field_df['is_validated'].sum():,} ({field_df['is_validated'].sum()/len(field_df)*100:.1f}%)" if len(field_df) > 0 else "0",
    ...
    try:
        ...
        if enable_ai and os.getenv("OPENAI_API_KEY"):
            ...
            if field_ai_summary:
                print(f"**Analisi AI per {field_name}:**\n\n{field_ai_summary}")
    except Exception:
        pass
...
if metrics_commentary:
    print(f"**Commento AI:** {metrics_commentary}")
```
...
if confusion_commentary:
    print(f"**Commento AI:** {confusion_commentary}")
```
...
if confidence_commentary:
    print(f"**Commento AI:** {confidence_commentary}")
```
